...
import { editMaintenance } from '../../../http/maintenanceApi';
...
export const MaintenanceEditModal = ({ show, onHide, item, fetch }: IProps) => {
    const {
        control,
        handleSubmit,
        reset,
        formState: { errors },
      } = useForm<IMaintenanceEditData>();
      ...
      const onSubmit = async (data: IMaintenanceEditData) => {
        await editMaintenance(data.id, data)
          .then(() => {
            onHide();
            fetch();
          })
